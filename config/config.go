...
func Load() Config {
	...
	cfg.PostgresHost = cast.ToString(getValueOrDefault("POSTGRES_USER", "localhost"))
	cfg.PostgresUser = cast.ToString(getValueOrDefault("POSTGRES_USER", "postgres"))
	cfg.PostgresDatabase = cast.ToString(getValueOrDefault("POSTGRES_DATABASE", "market_app"))
